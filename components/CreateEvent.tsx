...
const handleCreate = async (hide, e, code, eventName, description, showSuccess, setCodeURL) => {
    
    const { data, error } = await supabase
    .from('event')
    .insert([
      { code: code, event_name: eventName, description: description },
    ])
    .select()
    ...
    return(
      <div className="flex flex-column px-8 py-5 gap-4" style={{ borderRadius: '12px', backgroundImage: 'radial-gradient(circle at left top, var(--primary-400), var(--primary-700))' }}>
          ...
          <div className="flex align-items-center gap-2">
              <Button label="Crear" severity='success' onClick={(e) => handleCreate(hide, e, code, eventName, description, showSuccess, setCodeURL)} className="p-3 w-full"></Button>
